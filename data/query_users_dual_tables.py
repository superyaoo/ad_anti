    ...
    try:
        ...
        if isinstance(result, dict):
            if 'data' in result:
                ...
                print(f"❌ API 返回错误: {error_msg[:200]}...")
                return pd.DataFrame()
        else:
            data = result
...
def query_table2(user_ids: List[str], partition: str, api_url: str) -> pd.DataFrame:
    """查询表2: 特征表"""
    print(f"\n{'='*70}")
    print(f"表2: 特征表 (ad_kuaishou_visitor_device_product_feature_di)")
    print(f"{'='*70}")
...
def merge_dataframes(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    ...
    if not df2.empty:
        ...
        # 找出重复的列名（除了 visitor_id）
        duplicate_cols = [col for col in df2.columns if col in df1.columns and col != 'visitor_id']
        
        if duplicate_cols:
            print(f"⚠️  发现重复列: {', '.join(duplicate_cols[:5])}{'...' if len(duplicate_cols) > 5 else ''}")
            # 只对重复的列添加前缀
            rename_dict = {col: f'table2_{col}' for col in duplicate_cols}
...
def main():
    ...
    print(f"{'='*70}")
    print(f"\n前 3 条数据 (只显示前15列):")
    print(df_final.iloc[:3, :15])
    
    print(f"\n列名列表 (共 {len(df_final.columns)} 列):")
    for i, col in enumerate(df_final.columns, 1):
        prefix = ""
        if col.startswith('table2_'):
            prefix = "[表2] "
        print(f"  {i:3d}. {prefix}{col}")
    
    # 保存结果
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    csv_file = f'user_info_dual_tables_{timestamp}.csv'
    df_final.to_csv(csv_file, index=False, encoding='utf-8-sig')
    print(f"\n💾 CSV 已保存: {csv_file}")
    
    excel_file = f'user_info_dual_tables_{timestamp}.xlsx'
    ...
    print(f"   表1贡献列数: 29")
    print(f"   表2贡献列数: {len(df_final.columns) - 29}")
