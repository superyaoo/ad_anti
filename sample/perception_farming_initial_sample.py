...
SELECT
  ...
  paied_cnt,
  refund_cnt,
  CASE WHEN paied_cnt = 0 THEN NULL
       ELSE ROUND(refund_cnt * 1.0 / paied_cnt, 4)
  END                                                               AS refund_rate,
  order_cnt,
  avg_order_yuan,
  gmv_yuan,
...
FROM (
  SELECT
    ...
    s.click_cnt,
    s.cost_yuan,
    CASE WHEN s.impr_cnt = 0 THEN 0
         ELSE ROUND(s.click_cnt * 1.0 / s.impr_cnt, 4) END        AS ctr,
    s.total_coin,
  ...
  FROM (
    ...
    ) c ON a.visitor_id = c.visitor_id
  ) s
  LEFT JOIN (
    SELECT
      visitor_id,
    ...
    GROUP BY visitor_id
  ) d ON s.visitor_id = d.visitor_id
  LEFT JOIN (
    SELECT
      visitor_id,
      COUNT(*)                                                      AS order_cnt,
    ...
    GROUP BY visitor_id
  ) o ON s.visitor_id = o.visitor_id
) t
WHERE
